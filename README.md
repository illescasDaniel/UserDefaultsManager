...
public extension UserDefaultsManager where Key == UserDefaultsKey {
	...
	public static let custom = UserDefaultsManager<Key>(defaults: UserDefaults(suiteName: "_swift-app-test__5")!)
}
...
```swift
// Create an enum conforming to `serDefaultsKey` and others conforming to `StringUserDefaultsKey`, `BoolUserDefaultsKey`, (double, float, date, data, etc)
